...
function removeArtist(array, index) {
  ...
  return `The artist at index ${id} has just been removed from the array`;
}
...
function lotsOfArt(artists) {
  ...
  for (let i = 0; i < artists.length; i++) {
    let currentPainter = artists[i];
    if (currentPainter.paintings >= 100) {
      newArray.push(currentPainter.name);
    }
  }
  return newArray;
}
